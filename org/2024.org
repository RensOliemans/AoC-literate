...
*This is archived*. Running these codeblocks from Org mode will likely not work right now, since the util functions are a bit tricky to use in the same session as the daily functions. For [[file:2025.html][2025]] I've moved to a slightly different structure and have just copied this file for archiving purposes. Anyway, here goes:
